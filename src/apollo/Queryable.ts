...
import {
  buildRawOperationFromFragment,
  buildRawOperationFromQuery
} from './util';
...
> extends ApolloCache<TSerialized> {
  ...
  diff<T>(options: Cache.DiffOptions): Cache.DiffResult<T | any> {
    const rawOperation = buildRawOperationFromQuery(
      options.query,
      options.variables
    );
    const { result, complete } = this._queryable.read(
      rawOperation,
      options.optimistic
    );
  ...
  transformForLink(document: DocumentNode): DocumentNode {
    ...
    return removeDirectivesFromDocument([{ name: 'static' }], document)!;
  }
