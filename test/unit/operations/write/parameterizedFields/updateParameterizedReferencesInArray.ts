...
describe(`operations.write`, () => {
  ...
  describe(`updates parameterized references in an array`, () => {
    ...
    it(`writes nodes for each entity`, () => {
      jestExpect(baseline.getNodeData('1')).toEqual({
        id: 1,
        name: 'Foo',
        extra: false
      });
      jestExpect(baseline.getNodeData('2')).toEqual({
        id: 2,
        name: 'Bar',
        extra: true
      });
      jestExpect(baseline.getNodeData('3')).toEqual({
        id: 3,
        name: 'Baz',
        extra: false
      });
    ...
    it(`updates nodes for each entity`, () => {
      ...
      jestExpect(snapshot.getNodeData('2')).toEqual({
        id: 2,
        name: 'Bar',
        extra: false
      });
      jestExpect(snapshot.getNodeData('3')).toEqual({
        id: 3,
        name: 'Baz',
        extra: true
      });
