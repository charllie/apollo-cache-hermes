...
describe(`operations.restore`, () => {
  describe(`cyclic GraphSnapshot`, () => {
    ...
    it(`correctly restores different types of NodeSnapshot`, () => {
      ...
      jestExpect(restoreGraphSnapshot.getNodeSnapshot('1')).toBeInstanceOf(
        EntitySnapshot
      );
      jestExpect(restoreGraphSnapshot.getNodeSnapshot('2')).toBeInstanceOf(
        EntitySnapshot
      );
    });

    it(`restores RootQuery GraphSnapshot from JSON serialization object`, () => {
      const rootGraphSnapshot =
        restoreGraphSnapshot.getNodeSnapshot(QueryRootId)!;
      ...
      jestExpect(rootGraphSnapshot.outbound).toEqual([
        { id: '1', path: ['foo'] }
      ]);
    ...
    it(`restores id='1' GraphSnapshot from JSON serialization object`, () => {
      ...
      jestExpect(fooGraphSnapshot.outbound).toEqual([
        { id: '2', path: ['bar'] }
      ]);
