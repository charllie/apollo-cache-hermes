...
describe(`operations.restore`, () => {
  describe(`nested parameterized references in array`, () => {
    ...
    it(`correctly restores different types of NodeSnapshot`, () => {
      ...
      jestExpect(
        restoreGraphSnapshot.getNodeSnapshot(parameterizedId0)
      ).toBeInstanceOf(ParameterizedValueSnapshot);
      jestExpect(restoreGraphSnapshot.getNodeSnapshot('30')).toBeInstanceOf(
        EntitySnapshot
      );
      jestExpect(
        restoreGraphSnapshot.getNodeSnapshot(parameterizedId1)
      ).toBeInstanceOf(ParameterizedValueSnapshot);
    ...
    it(`restores parameterized NodeSnapshot in an array at index=0 from JSON serialization object`, () => {
      const parameterizedElement0 =
        restoreGraphSnapshot.getNodeSnapshot(parameterizedId0)!;
      const entityElement0 = restoreGraphSnapshot.getNodeData('30');

      jestExpect(parameterizedElement0.inbound).toEqual([
        { id: QueryRootId, path: ['one', 'two', 0, 'three'] }
      ]);
      jestExpect(parameterizedElement0.outbound).toEqual([
        { id: '30', path: [] }
      ]);
    ...
    it(`restores parameterized NodeSnapshot at index=1 from JSON serialization object`, () => {
      const parameterizedElement1 =
        restoreGraphSnapshot.getNodeSnapshot(parameterizedId1)!;
      const entityElement1 = restoreGraphSnapshot.getNodeData('31');

      jestExpect(parameterizedElement1).toBeInstanceOf(
        ParameterizedValueSnapshot
      );
      jestExpect(parameterizedElement1.inbound).toEqual([
        { id: QueryRootId, path: ['one', 'two', 1, 'three'] }
      ]);
      jestExpect(parameterizedElement1.outbound).toEqual([
        { id: '31', path: [] }
      ]);
