  ...
  describe(`with resolver redirects`, () => {
    ...
    it(`follows resolver redirects on the query root`, () => {
      const { result, complete } = read(
        context,
        { ...rootRedirectQuery, variables: { id: 1 } },
        baseSnapshot
      );
      expect((result as any).thing).to.deep.eq({
        __typename: 'Thing',
        id: 1,
        name: 'One'
      });
    ...
    it(`follows resolver redirects on the query root`, () => {
      const { result, complete } = read(
        context,
        { ...nestedRedirectQuery, variables: { id: 1 } },
        baseSnapshot
      );
      expect((result as any).nested.thing).to.deep.eq({
        __typename: 'Thing',
        id: 1,
        name: 'One'
      });
    ...
    it(`supports redirects to nowhere`, () => {
      const { result, complete } = read(
        context,
        { ...rootRedirectQuery, variables: { id: 123 } },
        baseSnapshot
      );
